...
{% if target.method == 'session' %}
def get_spark_df(identifier):
    # Strip catalog component and quotes for Spark Connect / EMR Serverless compatibility.
    # ref/source identifiers arrive as '"catalog"."schema"."table"'; spark.table() on a
    # Spark Connect session only accepts a 1- or 2-part name (schema.table).
    parts = identifier.replace('"', '').split('.')
    return spark.table('.'.join(parts[-2:]))

dbt = dbtObj(get_spark_df)
df = model(dbt, spark)
...
_spark_df_types = (pyspark.sql.dataframe.DataFrame, pyspark.sql.connect.dataframe.DataFrame)
if not isinstance(df, _spark_df_types):
  msg = f"{type(df)} is not a supported type for dbt Python materialization"
  raise Exception(msg)

{% if is_incremental() %}
df.write.mode("append").format("{{ config.get('file_format', 'parquet') }}").saveAsTable("{{ target_relation.schema }}.{{ target_relation.identifier }}")
{% else %}
df.write.mode("overwrite").format("{{ config.get('file_format', 'parquet') }}").option("overwriteSchema", "true").saveAsTable("{{ target_relation.schema }}.{{ target_relation.identifier }}")
{% endif %}
{% else %}
...
# how to execute python model in notebook (session method)
# def get_spark_df(identifier):
#     parts = identifier.replace('"', '').split('.')
#     return spark.table('.'.join(parts[-2:]))
# dbt = dbtObj(get_spark_df)
